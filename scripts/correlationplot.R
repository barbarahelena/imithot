...
selmet <- read.csv("results/metabolomics/lmer/lmm_results.csv", sep = ";")
microb <- read.csv("results/microbiome/lmer/lmm_results.csv", sep = ";")
selected_metabolites <- selmet |> filter(group2 == 6 & sig != "") |> pull(metname)
selected_microbes <- microb |> filter(sig != "") |> pull(taxname)
met_cols <- colnames(met)
mb_cols <- colnames(mb)
met_subset <- met[, selected_metabolites, drop = FALSE]
rownames(met_subset) <- met$SampleID
...
meta_subset <- meta %>% filter(SampleID %in% common_samples)
...
neg_color <- nejm_colors[7]  # Dark blue 
pos_color <- nejm_colors[5]  # Purple
col_fun <- colorRamp2(c(-0.5, 0, 0.5), c(neg_color, "white", pos_color))
...
pdf("results/metab_microb_correlations.pdf", width = 10, height = 13)
